...
## 前提: スプラッシュは Plymouth とアプリの 2 段構え

最初に全体の設計を押さえておきます。「1 つのスプラッシュ」に見せたいものを、実際には **2 つの実装で前半・後半に分担**します。

```
[前半] Plymouth テーマ          [後半] アプリ内の QML スプラッシュ      [本体 UI]
       OS ブート中を担当                アプリ初期化中を担当            ロード完了後に
       (アプリはまだ存在しない)         (重い UI を裏でロード)          クロスフェード
```

なぜアプリ側に**も**スプラッシュを実装するのか。アプリの本体 UI(画像・フォント・QML モジュールを大量に抱えた画面)は、プロセスが起動してから初回フレームまでに時間がかかります。そこで、エントリの QML は **QtQuick だけで完結する軽いスプラッシュ**にして即座に最初のフレームを出し、本体 UI は背後で遅延ロードして、準備ができたらクロスフェードします(実装は後述)。

このとき前半と後半が**同じデザイン・同じアニメーション**なら、ユーザーには切れ目のない 1 つのスプラッシュに見えます。本デモでは、背景画像・スプライト・アニメの軌道テーブルを 1 つの生成スクリプトから **Plymouth テーマと QML の両方へ出力**して、見た目を完全に一致させています。

残る課題が、まさにこの記事の主題である継ぎ目の 2 点です。

1. **前半 → 後半の切り替わりで黒い画面を挟まない**(解法 1)
2. **前半のアニメーションの続きから後半を再生する**(解法 2)
...
DEACTIVATE のおかげで plymouthd が生きており、FB も生きているので、(2) が可能になるわけです。読み戻しはカメラ撮影と違って**ピクセル値が整数のまま正確に取れる**(ノイズゼロ)ため、マーカーは背景より数 LSB 明るいだけで十分です。LSB(最下位ビット)は 8bit 色の最小階調のことで、本デモのマーカーは背景 `(0x07, 0x09, 0x0d)` 付近に対して `(0x12, 0x14, 0x1e)` — **各チャネル +8〜+11/255(約 4%)** しか違いません。暗部のこの差は 3px の帯では目視できませんが、デジタル読み取りなら +1 でも判別できます(閾値 +5 は背景グラデーションへのマージン)。
...
冒頭の「前提」で述べた後半部分です。最初のフレームを軽くするため、エントリの QML は QtQuick だけで完結させ、重い本体 UI は背後で [Loader](https://doc.qt.io/qt-6/qml-qtquick-loader.html) の遅延ロードにしてクロスフェードします。
